...
That means card 1 is worth 8 points (1 for the first match, then double
...
def solve(
    ...
    for line in puzzle.splitlines():
        (card_name, numbers) = line.split(sep=":", maxsplit=1)
        p = parse("Card {:>d}", card_name)
        assert isinstance(p, Result), f"{line} doesn't match"
        card_num = p[0]
        ...
        if card_score == 0:
            ...
            if part == "a":
                LOG.info(
                    "%2d matches for card %d, so score is %d",
                    card_score,
                    card_num,
                    pow(2, card_score - 1),
                )
                total_score += pow(2, card_score - 1)
            else:
                # For each of the following WIN cards, add a number of cards
                # equal the the number of copies of the current card
                for win in range(card_score):
                    card_count[card_num + win + 1] += card_count[card_num]

    if part == "b":
        total_score = sum(card_count.values())
    return total_score
