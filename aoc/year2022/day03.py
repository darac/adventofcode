...
def solve(input: str, part: Literal["a", "b"], runner: bool = False) -> Optional[int]:
    ...
    for line in input.splitlines():
        ...
        if part == "a":
            ...
            rucksack_a.update(list(line[:rucksack_size]))
            rucksack_b.update(list(line[rucksack_size:]))
            common = list(rucksack_a.intersection(rucksack_b))
        else:
            if len(rucksack_a) == 0:
                rucksack_a.update(list(line))
            elif len(rucksack_b) == 0:
