...
def solve(
    ...
    for line in puzzle.splitlines():
        ...
        count = int(line[1:])
        if direction == "L":
            if part == "b" and pointer - count <= 0:
                zero_count += abs(pointer - count) // 100 + (pointer != 0)
            pointer = (pointer - int(count)) % 100
            if part == "a" and pointer == 0:
                LOG.debug("--> Zero")
                zero_count += 1
            LOG.debug(
                "Left %d -> %d, count = %d", count, pointer, zero_count
            )
        elif direction == "R":
            if part == "b" and pointer + count > 99:
                zero_count += (pointer + count) // 100
            pointer = (pointer + int(count)) % 100
            if part == "a" and pointer == 0:
                LOG.debug("--> Zero")
                zero_count += 1
            LOG.debug(
                "Right %d -> %d, count = %d", count, pointer, zero_count
            )
        assert 0 <= pointer <= 99
