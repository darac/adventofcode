...
def navigate_from_node(
    ...
    for instruction in cycle(instructions):
        ...
        step_count += 1
        if (PART == "a" and position == "ZZZ") or (
            PART == "b" and position.endswith("Z")
        ):
            break
    return step_count
...
def solve(
    ...
    for line in lines:
        ...
        if not isinstance(p, Result):
            ...
            continue
        assert isinstance(p, Result), f"No match for {line}"
        nodes[p[0]] = {"L": p[1], "R": p[2]}

    LOG.info(
        "Loaded %d instructions and %d nodes", len(instructions), len(nodes)
    )

    # First, get the number of followers. That's equal to the number of
    # matching start nodes.
    followers = [
        position
        for position in nodes
        if (PART == "a" and position == "AAA")
        or (PART == "b" and position.endswith("A"))
    ]
    LOG.info("I have %d followers", len(followers))

    total_steps = 1

    for node in nodes:
        if (PART == "a" and node == "AAA") or (
            PART == "b" and node.endswith("A")
        ):
            steps = navigate_from_node(instructions, nodes, node)
            LOG.debug(
                "Steps to reach a terminal node from %s: %d", node, steps
            )
