...
def invalid_id(gift_id: str, check_len: int) -> bool:
    """Check if a Gift ID is invalid.

    Take the first `check_len` characters of `gift_id`.
    If ALL `check_len`-length chunks of `gift_id` match
    this, then the ID is invalid

    Args:
        gift_id (str): A numerical Gift ID
        check_len (int): How many characters to check

    Returns:
        bool: True if the Gift ID consists only of repeated strings
    """
    check_seq = gift_id[:check_len]
    LOG.debug("    Check %s for %s", gift_id, check_seq)
    if len(gift_id) % check_len != 0:
        return False
    return all("".join(g) == check_seq for g in batched(gift_id, check_len))
...
def solve(
    ...
    for id_range in puzzle.split(","):
        ...
        LOG.debug("  %s -> (%d,%d)", id_range, lower_bound, upper_bound)
        for gift_id in map(str, range(lower_bound, upper_bound + 1)):
            if (
                part == "a"
                and len(gift_id) % 2 == 0
                and invalid_id(gift_id=gift_id, check_len=len(gift_id) // 2)
            ):
                LOG.debug("      %s is a valid ID", gift_id)
                valid_ids.add(int(gift_id))
            elif part == "b":
                for check_len in range(len(gift_id) // 2):
                    if invalid_id(gift_id=gift_id, check_len=check_len + 1):
                        LOG.debug("      %s is a valid ID", gift_id)
                        valid_ids.add(int(gift_id))
