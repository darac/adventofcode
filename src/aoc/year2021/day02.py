...
def solve(
    ...
    LOG.info("Captain : Helmsman! Prepare to receive orders!")

    position = 0
    depth = 0
    aim = 0

    for command, count in [n.split() for n in puzzle.splitlines()]:
        LOG.info("Captain : %s %s", command, count)
        match command:
            case "forward":
                LOG.info(
                    "Helmsman: Aye-aye, sir. Moving Forward %d", int(count)
                )
                position += int(count)
                if part == "b":
                    depth += aim * int(count)
            case "up":
                if part == "a":
                    LOG.info(
                        "Helmsman: Aye-aye, sir. Coming Up %d", int(count)
                    )
                    depth -= int(count)
                    if depth == 0:
                        LOG.info("COMPUTER: Surfaced")
                    elif depth < 0:
            ...
            case "down":
                if part == "a":
                    LOG.info(
                        "Helmsman: Aye-aye, sir. Going Down %d", int(count)
                    )
                    depth += int(count)
                else:
                    LOG.info(
                        "Helmsman: Aye-aye, sir. Lowering Aim %d",
                        int(count),
                    )
                    aim += int(count)
    ...
    LOG.info(
        "Captain : Thank you, sailor. Yeoman, record that as %d",
        position * depth,
    )
