...
def solve(
    ...
    for match in re.finditer(
        r"(mul\((?P<left>\d{1,3}),(?P<right>\d{1,3})\)|(?P<toggle>do(n't)?\(\)))",
        puzzle,
    ):
        LOG.debug("Match: %s => %s", match.group(0), match.groupdict())
        if match.group("toggle"):
            if part == "b":
                enabled = match.group("toggle") == "do()"
                LOG.debug("Enabled: %s", enabled)
            # The trick is that we still need to skip this command in part A
            continue
        if enabled:
            LOG.debug(
                "%s * %s",
